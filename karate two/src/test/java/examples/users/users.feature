Feature: Sample API Test
  ...
  Scenario: Consultar la mascota ingresada previamente (Búsqueda por ID)
    Given url 'https://petstore.swagger.io/v2/pet/121'
    When method GET
    Then status 200
    And match response.name == '#present'
    And match response.name == 'Doge'
    And match response.id == 121
  ...
  Scenario: Consultar la mascota modificada por estatus (Búsqueda por estatus)
    ...
    When method GET
    Then status 200
    And match response[10].name == '#present'
    And match response[10].name == 'Doge Two'
    And match response[10].status == 'sold'
